...
export class ParticleSystem{
 ...
 spawnDebrisDust(x,y,intensity=1){
  const count=Math.min(28,6+Math.floor(Math.max(0,intensity)*8));
  for(let i=0;i<count&&this.items.length<WORLD.maxParticles;i++)this.items.push({type:"dust",x:x+(Math.random()-.5)*24,y:y+(Math.random()-.5)*12,vx:(Math.random()-.5)*65,vy:-15-Math.random()*70,life:.5+Math.random()*1.1,maxLife:1.6,radius:1.5+Math.random()*4});
 }
 ...
 update(dt,water,terrain){
  const next=[];for(const p of this.items){p.life-=dt;if(p.life<=0)continue;p.vx+=(p.type==="foam"?.22:.1)*(water?.atmosphereWindPx||0)*dt;p.vy+=(p.type==="foam"?115:p.type==="dust"?90:250)*dt;p.x+=p.vx*dt;p.y+=p.vy*dt;if(p.x<-20||p.x>WORLD.width+20||p.y>WORLD.height+20)continue;
